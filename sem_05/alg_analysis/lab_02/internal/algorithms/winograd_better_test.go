...
func TestWinogradBetterMulMatrix(t *testing.T) {
	type args struct {
		m1 matrix.Matrix
		m2 matrix.Matrix
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := WinogradBetterMulMatrix(tt.args.m1, tt.args.m2)
			if (err != nil) != tt.wantErr {
				t.Errorf("WinogradBetterMulMatrix() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("WinogradBetterMulMatrix() got = %v, want %v", got, tt.want)
			}
		})
	}
}
